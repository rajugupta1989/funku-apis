...
class UserPromoter(CommonAbstractModel):
    user = models.ForeignKey(User, on_delete=models.CASCADE, unique=True)
    profile_image = models.ForeignKey(FileRepo, on_delete=models.CASCADE, blank=True, null=True)
    promoter_name = models.CharField(max_length=100, blank=True, null=True)
...
class PromoterSocialProfile(CommonAbstractModel):
    promoter = models.ForeignKey(UserPromoter, on_delete=models.CASCADE, unique=True)
    promoter_gallery = models.ManyToManyField(FileRepo, blank=True, related_name="promoter_gallery") 
    ...
    promoter_additional_feature = models.ManyToManyField(AdditionalFeature, blank=True, related_name="promoter_additional_feature")
